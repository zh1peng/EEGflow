...
%REGISTER_NEW_OP Create a rest registry and add/override an op.
%
% Usage:
%   reg = rest.register_new_op('my_step', @my_step);
%   reg = rest.register_new_op(reg, 'my_step', @my_step);
%   reg = rest.register_new_op(reg, 'my_step', @my_step, 'AllowOverride', true);

    if nargin < 2
        error('rest:register_new_op:MissingArgs', 'Need (op, fn) or (reg, op, fn).');
    end

    if isa(op, 'containers.Map')
        reg = op;
        if nargin < 3
            error('rest:register_new_op:MissingArgs', 'Need (reg, op, fn).');
        end
        opName = fn;
        fnHandle = varargin{1};
    ...
    if ~isa(fnHandle, 'function_handle')
        error('rest:register_new_op:BadFn', 'fn must be a function_handle.');
    end
    ...
    if reg.isKey(opKey) && ~allowOverride
        error('rest:register_new_op:DuplicateOp', ...
