...
%
% Supported metrics: mean, peak_positive, peak_negative, peak_absolute, rms.
    ...
    if numel(contrast) == 2
        ...
        value = local_metric(a.avg(:, tmask), R.Metric) - local_metric(b.avg(:, tmask), R.Metric);
        ...
        value = local_metric(c.avg(:, tmask), R.Metric);
...
function v = local_metric(X, metric)
    switch metric
        case 'mean'
            v = mean(X, 2, 'omitnan');
        case 'peak_positive'
